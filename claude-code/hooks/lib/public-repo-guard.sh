...
_warn_private_terms_in_staged_diff() {
  ...
  # term list: social-hit static + private-name 動的 list を merge
  local terms=() term
  if [[ -f "$_social_hit_rule_file" ]]; then
    while IFS= read -r term; do
      [[ -z "$term" ]] && continue
      terms+=("$term")
    done < <(_extract_term_list "$_social_hit_rule_file" "social-hit (block)")
  ...
  printf '⚠ staged diff に private term の疑い: [%s]。ai-tools は public repo のため、commit 前に git diff --cached で該当行を確認し匿名化 / 削除を検討してください (warn のみ、block しません)' "$hit"
}
