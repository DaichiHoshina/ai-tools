...
各 phase doc は前 phase doc を `## 1. Overview` の "PRD link / reference" 行から参照する。後 phase が前 phase の決定を覆す場合、前 phase doc 側を `--update <path>` で同期する (`--scope` は PRD Q1-Q5 partial fix 用途、Design Doc section 指定には使わない)。
