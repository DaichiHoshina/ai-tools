...
| Root cause analysis, design decisions, complex bug analysis, security audit | Opus 4.8 | `claude-opus-4-8` | `/model` → opus |
...
**Policy**: parent (chat) orchestrates with Opus 4.8; subagents split into judgment=Opus 4.8 / execution=Sonnet 4.6 (changed from unified-Sonnet policy on 2026-06-05 to improve judgment quality).

- **Opus 4.8 (judgment subagents)**: po-agent (strategy / design decisions), manager-agent (task decomp / parallelism calculation), reviewer-agent (12-perspective review / quality judgment), root-cause-analyzer (complex bug analysis / 5 Why)
- **Sonnet 4.6 (execution subagents)**: developer-agent (impl / refactor), explore-agent (read-only exploration), verify-app (build / test execution)
...
> `xhigh` is Opus-only (v2.1.111+; check `claude --help` `--effort` choices). Opus 4.8 default effort is `high`. Other models fall back to `high`.
