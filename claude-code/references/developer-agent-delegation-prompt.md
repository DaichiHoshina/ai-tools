...
**Canonical**: `guidelines/writing/code-comment.md`. Agent must Read canonical **before** adding/editing any `// ` `# ` `-- ` `/* ` `<!-- ` line. Delegation prompt MUST include this Read directive as 1 line.

Rules (canonical 抜粋、full は必ず canonical Read):

- Default = 書かない。identifier 名で読める内容は書かない
- 残す 3 分類のみ: (1) WHY 設計判断根拠 / (2) 重要 memo (業務ルール / 隠れた制約 / incident workaround) / (3) godoc-style 公開シンボル説明
- 削除 9 カテゴリ: what 言い換え / PR 文脈依存 (チケット番号単独) / 自明アサーション / defensive 言い訳 / 主観評価 / テスト不確実 / 重複 / 経過メモ (「〜から変更」「以前は〜」等の時系列記述) / commented-out code
- **現在の事実のみ記載**: 開発中の変遷・経緯は git history / PR が保持する。comment には現在のコードの WHY だけ書く
- AI marker 禁止 (`// AI-generated` / `// Generated by Copilot` 等)
- Comment Traps 回避: 実装と乖離した stale comment / commented-out code を残さない
- 擬人化 NG (「flag が〜する」「エラーが〜する」)
- task / PR / caller 参照禁止 (「for X」「issue #N fix」→ PR description に書く)
- 多段落 docstring / 多行 comment block 禁止。書くなら 1 行
- 日本語 comment は主語明示 + 半角スペース + 二重否定回避 (canonical 「日本語品質」節)

Self-review: 提出前に追加 comment を canonical 13 分類 table で自己分類し、削除 9 に該当するものを削除する。confidence < 80% の判定は discard する。
