...
| **worktree applicability** | `worktree_required` と downgrade 条件 (`references/PARALLEL-PATTERNS.md#worktree-applicability-flow`) の整合 |
...
aggregate 後、N 本の dev 差分を parent Opus が 4 観点で再評価。

| 観点 | 検査 |
|---|---|
| **cross-diff conflict** | 同一 file 同一行を複数 dev が編集していないか (`git diff` 行重複) |
| **duplicate import / decl** | aggregate 後 file で import / type / const の重複追加が無いか |
| **naming collision** | 別 dev が同名 symbol を別意味で導入していないか |
| **propagation incompleteness** | dev A の interface 変更を dev B の caller が追従できているか (Serena `find_referencing_symbols`) |
