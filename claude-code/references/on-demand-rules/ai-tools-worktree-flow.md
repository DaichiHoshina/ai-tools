...
- `git stash drop` / `git stash clear` は permission denied で AI から実行できない。stash 削除は user に `! git stash drop` を session 内で打ってもらう (`!` prefix で出力が会話に入る)。代替 command での回避は deny-rule no-escalation で禁止 (`[[feedback-deny-rule-no-escalation]]`)
