...
| `skills/comprehensive-review/skill.md` | iter2 | Pattern 2 精緻化 (ツール不在判定を PATH 探索→実行結果ベースに変更、`npx`/`npm run` の node_modules/.bin 解決を考慮、判定優先順位 4 段表化) — Codex 2 ループで P0/P1/P2 全消化 |
...
| `commands/brainstorm.md` | iter1 | Pattern 2 (Superpowers 未導入時 /plan 委譲 / 対立検証 Agent 片方/両方失敗時の降格) |
| `commands/review-fix-push.md` | iter1 | Pattern 3 (ループ進捗・未収束時の出力フォーマット追加) |
| `commands/refactor.md` | iter1 | Pattern 2 (Serena / ガイドライン読込失敗 + 改善余地ゼロ件 + テスト失敗時 stash 退避) |
| `commands/performance-issue.md` | iter1 | Pattern 1 (CPU/I/O 中間判定行追加) + Pattern 2 (issue 取得不能 / pprof 失敗 / 改善後悪化検出時の挙動) |
| `commands/test.md` | iter1 | Pattern 2 (フレームワーク検出失敗時停止 / カバレッジ未達 / Serena 失敗 / 既存テスト破壊検出) |
| `commands/retrospective.md` | iter1 | Pattern 2 (history.jsonl 不在 / Serena 接続失敗 / データ不足 / 全失敗時の手動振り返り案内) |
| `commands/memory-save.md` | iter1 | Pattern 2 (serena 失敗時ローカル fallback / 書込失敗時チャット出力 / memory 名衝突時 suffix 付与) |
| `commands/explore.md` | iter1 | Pattern 2 (explore-agent 全件失敗時 grep 代替 / 部分失敗時の集約方針 + 未カバー明示 / Serena 失敗時 find 降格) |
| `commands/verify-once.md` | iter1 | Pattern 2 (bash/jq/bats 不在時 skip / 統合テスト未整備時停止 / install 反映後失敗時 rollback) + Pattern 3 (失敗時の出力例追加) |
| `commands/protection-mode.md` | iter2 | 指摘ゼロ収束 (codex 1 ループ、no patch) — iter1 で完成度十分と確認 |
| `skills/load-guidelines/skill.md` | iter2 | Pattern 2 精緻化 (ゼロ件 fallback を summary/full モード別に分岐、full 時 common 3 本 mandatory 保持) — codex 1 ループで収束 |
| `commands/git-push.md` | iter2 | 指摘ゼロ収束 (codex 1 ループ、no patch) — iter1 で完成度十分と確認 |
| `agents/po-agent.md` | iter2 | Pattern 2 精緻化 (Worktree 「作成しない」を main 固定→現ブランチ続行に修正、判断不能を「自動デフォルト」→「親への返却前確認必須」に変更) — codex 1 ループで収束 |
| `agents/manager-agent.md` | iter2 | Pattern 2 精緻化 (品質基準 fallback の P0-P3 参照先を reviewer-agent §レビュー観点に明示、worktree 情報欠落時を main 仮定→現ブランチ継続に修正) — codex 1 ループで収束 |
| `agents/explore-agent.md` | iter2 | 指摘ゼロ収束 (164afdf 全体 review で actionable issue なし、worktree fallback 既に正しい記述) — iter1 で完成度十分と確認 |
| `agents/root-cause-analyzer.md` | iter2 | 指摘ゼロ収束 (164afdf 全体 review で actionable issue なし) — iter1 で完成度十分と確認 |
| `agents/developer-agent.md` | iter2 | Pattern 2 精緻化 + Compounding 拡張 (Serena 失敗時 fallback の例外明記で必須使用セクションとの矛盾解消、worktree 未指定時の現ブランチ継続 + main 系作業時 WARN 必置) — codex 2 ループで収束 |
| `commands/dev.md` | iter2 | Pattern 2 精緻化 + Compounding 拡張 (`/dev --parallel --auto` の「条件」と「挙動」混在を整理: 3 条件 + worktree 作成失敗時フォールバック明示、見出し整合) — codex 2 ループで収束 |
| `commands/review.md` | iter2 | 指摘ゼロ収束 (da0ee83 全体 review で actionable issue なし) — iter1 で完成度十分と確認 |
| `commands/diagnose.md` | iter2 | 指摘ゼロ収束 (da0ee83 全体 review で actionable issue なし) — iter1 で完成度十分と確認 |
| `commands/lint-test.md` | iter2 | 指摘ゼロ収束 (da0ee83 全体 review で actionable issue なし) — iter1 で完成度十分と確認 |
| `commands/plan.md` | iter2 | 指摘ゼロ収束 (fallback table が L141-144 で網羅済、Compounding 観点でも整合) — iter1 で完成度十分と確認 |
| `commands/prd.md` | iter2 | 指摘ゼロ収束 (da0ee83 全体 review で actionable issue なし) — iter1 で完成度十分と確認 |

## iter1 全体進捗 (2026-05-01)

- skills: 21/21 適用完了
- commands: 主要 17 件適用完了
- agents: 7 件適用完了
- 残 commands (analytics / audit / docs / serena / serena-refresh / aliases / dashboard / claude-update-fix / flow / flow-auto / groove / git-pull / post-comment / design-doc / update-guidelines / test-local / ui / skill-add / skills-manage / reload): 局所利用 or 極小 shortcut で iter1 優先度低、iter2 候補からも除外

## iter2 進捗

- comprehensive-review (2 ループ収束、Pattern 2 精緻化)
- protection-mode (1 ループ収束、修正不要)
- load-guidelines (1 ループ収束、Pattern 2 精緻化: ゼロ件 fallback モード分岐)
- git-push (1 ループ収束、修正不要)
- po-agent (1 ループ収束、Pattern 2 精緻化: Worktree 既存ブランチ保護 + 判断不能時の事前確認)
- manager-agent (1 ループ収束、Pattern 2 精緻化: P0-P3 参照先明示 + worktree 欠落時の現ブランチ継続)
- explore-agent (1 ループ収束、修正不要: 164afdf 全体 review で actionable なし)
- root-cause-analyzer (1 ループ収束、修正不要: 164afdf 全体 review で actionable なし)
- developer-agent (2 ループ収束、Pattern 2 精緻化 + Compounding 拡張: Serena fallback 例外明記 + main 系作業時 WARN 必置)
- dev (2 ループ収束、Pattern 2 精緻化 + Compounding 拡張: --parallel --auto の条件/挙動分離 + 見出し整合)
- review / diagnose / lint-test / plan / prd (5 件まとめ、surface review で指摘ゼロ収束、Compounding 観点でも整合)
- 残 iter1 適用済 32 件: 使用頻度順 (post-tool-use 計測) で順次着手

## iter2 既知の残課題 (commit 単位)

なし（dev.md L36/L38/L44 で見出し・本文・フォールバック明示の 3 連鎖修正完了）

## Compounding Engineering 観点メモ

- po-agent / manager-agent / developer-agent で「worktree 情報欠落時 = main 仮定」が同パターン問題として連続発見
- 全 3 件で「現ブランチ継続 + 仮定しない」修正を横断適用済
- 教訓: Worktree fallback を agent 定義に書く際は **PO 契約「未指定 = 現ブランチ継続」を起点**に統一表現を採用すべし
