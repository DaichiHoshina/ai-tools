...
- **deprecated model warning が agent frontmatter もカバー** [2.1.183]: deprecated / auto-updated model を stderr 警告 (`-p` mode + agent frontmatter)。現 agent frontmatter の model ID (opus-4-7 / sonnet-5) は全て valid のため警告対象外。opus-4-7 は Opus 4.8 regression 回避の意図的 pin (`[[work-context-20260616-opus-4-8-regression-and-full-audit]]`)、変更しない。
