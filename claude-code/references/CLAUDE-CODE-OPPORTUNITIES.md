...
- **Claude Sonnet 5 default model 化** [2.1.197]: Sonnet 5 が Claude Code の default model に昇格、native 1M-token context、$2/$10 per Mtok promo (~08/31)。既存 agent frontmatter (`claude-sonnet-4-6` × 5 files, `claude-opus-4-7` × 4 files) と `templates/settings.json.template` の `availableModels` / `fallbackModel` は現行 opus-4-7 pin (opus-4-8 regression 回避、[[work-context-20260616-opus-4-8-regression-and-full-audit]]) を維持。sonnet-4-6 → sonnet-5 の一括切替は auto 判定で試験 → 品質確認後に採用したい。CLAUDE.md § Auto-Delegation の `/model sonnet` alias 記述は最新 sonnet を指すため影響なし。Opportunity。
