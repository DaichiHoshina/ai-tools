...
- **Large-repo session split**: Hard reset (`/clear` or new session) at task boundary; never carry session past 1 task / `$_TH_SESSION_AGE_S` sec elapsed / `$_TH_SESSION_MSG` msg / 40% context (whichever first). Canonical values: `hooks/lib/thresholds.sh`. 1 task = 1 session principle in large repos
