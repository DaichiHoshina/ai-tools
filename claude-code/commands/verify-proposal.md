...
description: 提案 (brainstorm/agent report/plan の候補 list) を実物と突き合わせ、改善案を出し、reviewer-agent で第 3 者 review する 1 pass loop
argument-hint: "[proposal file path / plan file / chat 直貼りの item list]"
---

# /verify-proposal - 提案の実物突き合わせ + 改善 + review
...
| **`/verify-proposal`** | brainstorm / agent 出力を実物と突き合わせて採否判定する (本 command) |
| `/grill` | 設計案の穴を炙る (前提監査、read-only、質問生成のみ) |
| `/plan` | verified な案を Phase 分解する |
| `/verify-once` | 実装後の syntax / test 通し (別 layer) |

順序: `/brainstorm` → **`/verify-proposal`** → `/plan` → `/dev` → `/verify-once`

`/grill` との差: grill は「前提が正しいか」を質問で炙る (read-only)、verify-proposal は「見積が実物と一致するか」を grep/wc で測る (実測)。両者は補完関係で、grill → verify-proposal の順で流すと厚みが出る。
...
- **1 pass 固定**: max 1 iteration。review で hold が出た場合の再 loop は `/verify-proposal` を user が手で再発火する (auto 循環は goal drift を招くため入れない)
