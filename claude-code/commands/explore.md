...
| 状況 | 動作 |
|------|------|
| explore-agent 全件失敗 | 直接 grep / Glob で代替探索、精度低下を warning。最低限のディレクトリ構造を報告 |
| 部分失敗（N 並列中 M 件成功） | 成功分のみで集約、失敗観点を「未カバー」と明示。ユーザーに再実行可否確認 |
| Serena MCP 失敗 | find / grep に降格、シンボル解決精度低下を warning |
