...
| file 対象の write / rewrite | `Task(developer-agent)` を 1 発火し、natural-japanese quick 工程 (lint → 判断台帳 → 書き直し → `--baseline` 再 lint) の収束ループを agent 内で完結させる (周回ごとの再発火は禁止。ループが agent 内側で閉じるため iteration inline 固定 rule と両立する)。prompt には house 規範参照 (PRINCIPLES / NG-DICTIONARY / `nominal_ending` 不採用 / `stats.nominal_ending_ratio` 0.2 閾値 / 聞き返さない) と 4-question の回答を渡し、diff + 最終 lint JSON + 5-Axis 採点を返させる。衝突時は house 規範 > natural-japanese。write の draft は agent が一時 file 経由で lint し、中間 file は削除して返す。agent / skill が使えない環境は本行を skip し、下記の現行経路で単独成立させる |
| file 対象の review / 委譲不発火時の rewrite | `references/on-demand-rules/natural-japanese-lint.md` を load し、parent 側で lint を Bash 実行して JSON を 5-Axis の補助入力に渡す (skill は Bash 禁止のため実行しない)。短文でも省略しない (禁止語 / 翻訳調は文 1 つでも検出される)。統計系 detector (文長リズム / 段落均質 / 語彙多様性) は外向き長文 doc のみ採用する |
