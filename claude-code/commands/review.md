...
| `--ultra` | built-in `/ultrareview` に委譲（cloud並列マルチエージェントレビュー、localトークン節約） |
| `--no-difit` | difit起動を抑制 |

**自動実行される内容**:

```
if --ultra:  → /ultrareview <引数>（Skill 呼ばずに終了）
else:        → Skill("comprehensive-review")
```
...
`/ultrareview <引数>` に委譲（v2.1.111〜、v2.1.113 で並列化高速化）。

- 引数なし → `/ultrareview`（現在ブランチ）
- PR番号 → `/ultrareview <N>`（指定PRをfetch→レビュー）
- cloud側で並列マルチエージェント実行、**localトークン消費なし**
- comprehensive-review skillは呼ばない（完全に built-in に委譲）

使い分け:
- 通常: `/review`（localトークン使用、詳細制御）
- 大規模差分・別視点が欲しい: `/review --ultra`
- codex セカンドオピニオン: `/review --codex`
