...
- PR URL必須（引数 or `gh pr view --json url -q .url` で自動取得）
- `npx devin-review {pr-url}` をバックグラウンド起動
- 初回のみブラウザで認証（GitHub App or PAT）。以降はキャッシュ
- 結果確認先: stdout or `devinreview.com/{owner}/{repo}/pull/{n}`（= `github.com` を置換）
- comprehensive-review と並列実行可能。結果統合は codex と同じ扱い（両方指摘=確度高）
