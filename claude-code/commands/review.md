...
/review --codex        # Codex CLIのセカンドオピニオン付き
/review --no-difit     # difit起動を抑制
```
...
| `--focus=<観点>` | 指定観点のみレビュー |
| `--codex` | comprehensive-review + `codex review`を並列実行、結果を統合 |
| `--no-difit` | difit起動を抑制 |
...
`--codex`指定時、comprehensive-reviewと並列で`codex review`を実行:
...
**実行フロー**:
1. comprehensive-review と `codex review --uncommitted` を**並列実行**
2. 両方の結果を統合:
   - Claude側の Critical/Warning と Codex側の指摘を突合
   - 両方が指摘 → **確度高**（必ず修正）
   - 片方のみ指摘 → 内容を精査して判断
3. 統合結果を出力

**codex未インストール時**: `which codex`で確認、なければCodex部分をスキップして通常レビューのみ実行。
...
ローカルレビュー時、デフォルトでレビュー完了後にdifitを起動してブラウザで差分+レビューコメントを表示。`--no-difit`で抑制。

### 動作条件

- ローカルレビュー（引数なし）時のみ有効。URL/番号指定時は無視
- `difit`コマンドが必要（`npm i -g difit`）

### 実行手順

1. 通常どおりレビュー実行（comprehensive-review）
2. findings（Critical/Warning）からdifit用コメントJSONを構築
3. `difit`をバックグラウンド起動
...
```json
{
  ...
  "body": "🔴 [設計] Domain→Infrastructure参照\n\n修正案: ..."
}
```
...
行番号が特定できないfindingは`line: 1`で該当ファイルに付与。

### difitコマンド構築

```bash
# staged diffの場合
difit staged --comment '<JSON配列>'

# all changesの場合
difit . --comment '<JSON配列>'
```

コメントは1つの`--comment`に全findingsをJSON配列で渡す。
