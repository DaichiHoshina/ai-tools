...
- **comment + writing check (always-on)**: diff に comment 行の追加/変更があれば `guidelines/writing/code-comment.md` 13 分類 check、prose diff (md/docs) があれば `guidelines/writing/PRINCIPLES.md` check を必ず実行する (canonical: `skills/comprehensive-review/SKILL.md` §Writing Enforcement)。skip 可否は comment/prose diff 0 の場合のみ
