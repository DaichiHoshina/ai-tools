...
| 3.5. **Writing Context ブロック必須記述**（新規） | draft 本文の冒頭に以下の HTML コメントブロックを置き、4問の答えを埋めてから Step 4 へ進む。物理的に書かないと次に進めない強制機構。最終提出時の削除は任意 |
| 4. draft 生成 | テンプレ12セクション（タイプ別調整、`references/design-doc-template.md` 準拠）。**Writing Context で書いた「読み手・アクション・数字・なぜ」を本文に織り込む**（頭で答えて終わりにしない） |
...
| 7.5. セルフチェック | `user-voice.md` 末尾の6項目（TL;DR・根拠併記・難語定義・具体性・次アクション・4問通過）。**5項目未満なら書き直し**|
...
## Writing Context ブロック（Step 3.5 で必須記述）

draft 本文の最上部（H1 の直前）に以下のブロックを置く。4問の答えを具体的に埋めてから本文を書く。答えが埋まっていない状態で Step 4 へ進まない。

```markdown
<!-- Writing Context（執筆前の4問・書き直しの根拠。最終提出時の削除は任意） -->
<!--
読み手: <具体的な人物像。例: snkrdunk バックエンドチーム senior engineer、レビュー担当の Ayano さん>
読後のアクション: <例: approve する / 実装に着手する / 懸念点を指摘する>
主要な数字・事例: <例: 1M 口で 509ms、legacy の 21倍、メモリ 4.7MB>
なぜこのドキュメントが必要: <問題 / PRD 接続。例: 現行抽選が 1M 口で 10.8s かかり PRD 目標達成不可>
-->
```

**抽象禁止**。「エンジニア」「ユーザー」「改善」だけでは失格。具体的な人物像・数字・問題を書く。埋めたら本文執筆に入り、Writing Context の内容を本文各所に織り込む。
