...
### Step 4.5: Writing Context ブロック必須記述（強制機構）

draft 冒頭に以下を置き、4問の答えを具体的に埋めてから Step 5 へ進む。Notion 投稿時は削除しても良い（保存用 md には残す）。

```markdown
<!-- Writing Context（執筆前の4問・書き直しの根拠） -->
<!--
読み手: <具体的な人物像。「エンジニア」だけは失格>
読後のアクション: <情報共有のみ / 再現手順実行 / 類似対応時に参照 等>
主要な数字・事例: <件数、時間、金額、失敗事例の具体>
なぜ残すのか: <ナレッジ化する動機>
-->
```

抽象禁止。Writing Context で書いた内容が本文に反映されないと自己矛盾なので、本文執筆時に織り込む。
