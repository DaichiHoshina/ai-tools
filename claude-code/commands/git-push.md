...
## Testing
<どう検証したか。実行した内容を具体的に。検証していない場合は正直に書く>
...
検証を行った場合は、実行したコマンド・環境・観測した結果を具体的に書く（「`go test ./...` pass、staging で p99 測定: 320ms」等）。

検証していない場合は、**捏造せず** 以下のいずれかを明記する:
- `Not run — docs-only 変更`
- `Not run — WIP、review 後に検証予定`
- `Manual only — UI で X 操作して Y を確認`
- `N/A — build 設定のみ`

**避ける**: 「本PRでは〜を実装しました」「改善しました」「最適化しました」（根拠・数字なし）、未実行テストの虚偽記載。
