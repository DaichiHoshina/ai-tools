...
## Step 0: タスクタイプ別 Skill / Agent マップ

以下のタスクタイプ判定表でワークフローと skill / agent が決まる。マッピング詳細は `references/command-resource-map.md` を参照。
...
3. **実装分岐**:
   ...
   - 直接 → `/dev`
4. **Team経路レビュー**（Team使用時のみ）: 親が `Task(reviewer-agent)` 起動（**`--codex` モード固定**、comprehensive-review + codex review 並列、両者共通指摘を優先）→ P0/P1 判定
...
5. **後続**: 判定表の *実装* 以降を順次実行（Team経路の `/review` は Step 4 で完了済なので skip）
...
worktree 適用判定詳細: `references/PARALLEL-PATTERNS.md#worktree 適用判定フロー` 参照。
