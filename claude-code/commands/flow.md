...
| 1 | 緊急, hotfix, 本番, production, critical | **緊急対応** | /diagnose → /dev → /lint-test → /git-push --pr |
| 1.5 | インシデント, 障害, incident, 調査して, エラーログ | **インシデント対応** | Skill(incident-response) → /diagnose → /dev → /lint-test → /git-push --pr |
| 2 | 根本, 原因分析, root cause, rca | **バグ修正（RCA付き）** | /diagnose → Skill(root-cause) → /dev → /lint-test → /git-push --pr |
| 3 | 修正, fix, バグ, エラー, 不具合, bug, error | **バグ修正** | /diagnose → /dev → /lint-test → /git-push --pr |
...
| 10 | トラブルシュート, troubleshoot, 調査, 診断 | **トラブルシュート** | /diagnose → /dev → /docs |
| 11 | その他 | **新機能実装** | （デフォルト） |
...
| レベル | 例 | フロー |
|--------|---|--------|
| Low | タイポ、インポートミス | /diagnose → /dev → /lint-test → /review → /git-push --pr |
| Medium | ロジックバグ、検証漏れ | /diagnose → Skill(root-cause) → /dev → /lint-test → /review → /git-push --pr |
| High | 競合状態、セキュリティ | /diagnose → Task(root-cause-analyzer) → /dev → /lint-test → /review → /git-push --pr |
...
## worktree並列実行（v2.1.104+）

`--worktree`と`--tmux`の組み合わせで、独立した作業環境を自動作成して並列実行が可能。

```bash
# 例: worktree + tmuxペインで並列作業
claude -w feat/auth --tmux
```

`/flow`での活用:
- `--auto`モード時、独立タスクを複数worktreeで並列実行可能
- 各worktreeは自動クリーンアップ（変更なしの場合）
- iTerm2ではネイティブペイン、それ以外は従来のtmuxを使用
