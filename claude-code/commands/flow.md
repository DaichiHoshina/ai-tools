...
```text
--skip-prd / --skip-test / --skip-review / --no-po / --interactive / --auto / --parallel / --orchestrate
```
...
`--parallel` と組合せ可、`--auto` とも互換。本 mode の目的は LLM 自発判断依存からの脱却 (incident `parallel-fire-format-peak-concurrency` 等の再発防止)。
