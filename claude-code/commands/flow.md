...
```bash
--simple        # Simple強制（進捗追跡・タイムアウトなし）
--complex       # TaskDecomposition強制（進捗追跡+2hタイムアウト）
--teams         # AgentHierarchy強制（全機能有効化）
```
...
**複雑度の明示指定**: 自動判定を上書きしたい場合に `--simple`/`--complex`/`--teams` を使用。指定なしの場合は自動判定。
...
**複雑度判定基準**:
- Low: タイポ、インポートミス、変数名ミス、単純な条件反転
- Medium: ロジックバグ、データ検証漏れ
- High: 競合状態、メモリリーク、セキュリティ脆弱性、データ破損、アーキテクチャ問題、繰り返し発生

### RCAフェーズの内容

1. 5つのなぜ分析で根本原因を特定
2. 修正戦略の比較（L1対症療法/L2部分的治療/L3根本治療）
3. 類似問題の検出
...
テスト:    /test → /review → /commit-push-pr
