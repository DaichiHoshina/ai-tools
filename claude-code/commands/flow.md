...
- **Team使用**: `Task(po-agent) → (PO自走で) Task(manager-agent) → Task(developer-agent)×N`
- **直接実行推奨**: `/dev`
...
PO Agentが「Team使用」or「直接実行推奨」を判断（デフォルト: Team）。`--no-po` 指定時のみPOをスキップし、`/dev` で直接実行（緊急時のみ）。

**重要**: PO が「Team使用」と判断した場合、**PO 内部で `Task(manager-agent)` を自走起動**し、Manager が `Task(developer-agent)` を並列起動する。親（Claude Code）は PO 完了を待つだけ。判定結果が「直接実行推奨」の時のみ `/dev` にフォールバック。

### Step 3: 実装ステップの分岐（*実装* の展開）

Step 2 の PO 判断結果で分岐:

- **Team使用** → PO が Manager/Developer を自走起動済み。親は Step 4 に進む
- **直接実行推奨** → 親が `/dev` を起動
- **`--no-po`** → 親が `/dev` を起動
