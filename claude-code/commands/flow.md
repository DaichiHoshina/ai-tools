...
8. **Parallel integrate + review** (fire both in 1 message): `Task(manager-agent)` integrate **and** reviewer fan-out simultaneously. Reviewer fan-out:
   ...
   - `--auto` (`/flow-auto`) または `--multi-review`: `Task(reviewer-agent, --focus=<lens>)` を 12 lens で **stage split** (Gate C、stage 1=6 lens + `--codex`=7 agent / stage 2=6 lens、8 Dev limit + 9 concurrent session limit + 境界余裕 1 確保)。各 reviewer は単一 lens 専念で深掘り、stage 間は parent Opus 待ち合わせ
