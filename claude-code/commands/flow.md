...
`--auto` 時、実装完了後に `/review` → 自動修正を **Critical 0件 + Warning 0件** になるまで繰り返す。

```
loop (最大3回):
  /review（comprehensive-review）
    → Critical 0 + Warning 0 → ループ終了 ✅
    → Critical > 0 or Warning > 0:
      → 各指摘を自動修正（/dev相当）
      → /lint-test（修正で壊れてないか確認）
      → 次のループへ
3回到達しても残る場合 → 残件をユーザーに報告して続行
```
...
```
実装完了 → /lint-test → /review → review-fix ループ → /git-push
```
