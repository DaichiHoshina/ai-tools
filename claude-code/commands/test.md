...
`/test --tdd` でTDDサイクルを強制:

```
RED（失敗テスト作成）→ GREEN（最小実装）→ REFACTOR（品質改善）→ 繰り返し
```

- テストファースト強制: 実装コードを先に書かない
- 最小実装: テストを通すための最小限のコードのみ
- 完了後 `/lint-test` で包括検証
...
| 状況 | 動作 |
|------|------|
| テストフレームワーク自動検出失敗（package.json/go.mod 等に該当なし） | ユーザーに使用フレームワーク明示要求して停止 |
| カバレッジ目標未達（< 70%） | 不足箇所の追加テスト案を出力、ユーザー判断で続行/打切り |
| Serena MCP 失敗 | grep / Read で関数シグネチャ取得、エッジケース検出精度低下を warning |
| 既存テスト破壊検出（実装変更を伴う場合） | 即停止、ユーザーに修正方針確認 |
