...
| 状況 | 動作 |
|------|------|
| CI 設定 + manifest 両方未検出 | 「CI 構成不在」と報告、ユーザーに `lint` / `test` コマンド明示要求して停止 |
| 推測したコマンドが存在しない | warning + skip、Result に「skipped (command not found)」記載 |
| 全 step skip された | Result: PASS でなく「N/A: 検証対象なし」と明示 |
