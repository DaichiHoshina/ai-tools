...
clear も**個別 file を必ず書く** (auto merge/new と同じ file format、ただし凝縮版で 30 行前後を目安)。MEMORY.md 1 行だけでは task / 直近 state / 実データ所在 / next-action を圧縮しきれず、次 session 復元時に AI が git log 推測 → user への scope 再質問を誘発するためだ。file 肥大化は `/memory-clean` の trash/prune で別途対処する。

## Save target dir

**常に `${HOME}/ai-tools/memory/` に固定**。cwd によらず変わらない。Claude Code / Codex / Cursor の 3 ツール共有 SoT (`~/ai-tools/memory/` は 3 ツールが symlink 経由で同じ場所を読む)。
...
1. **同 topic 同日 file 検出** (auto): `bash ~/.claude/scripts/memory-save-helper.sh list-today`
...
default (empty arg) と `$ARGUMENTS` に `clear` を含む時の動作:
