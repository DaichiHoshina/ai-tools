...
**Self-Review 3 gate は強制実行** (A: parallel-judgment / B: parallel-implementation / C: 12 観点 split review)。`/flow` `## Self-Review` section が canonical。`--auto` では Gate C (`--multi-review` 12 観点並列) が自動 ON。
