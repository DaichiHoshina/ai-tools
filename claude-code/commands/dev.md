...
### `/dev --parallel --auto` 確認スキップ条件 (3 条件 + 失敗時フォールバック)

`--auto` 併用時、以下 3 条件すべて満たす場合のみ確認をスキップ:

1. 直接実行判定式 PASS（`LPT_makespan + 21N + 20 < sum × 0.7`、`N=4 + T_task>58s` が第一候補、+ 独立 2 件 + 完全分離）
2. clean worktree
3. branch / worktree 名衝突なし

**worktree 作成失敗時のフォールバック**: 順次実行 (`N=1`、worktree なし、現在ブランチ) に自動降格、warning ログ出力。`--auto` であっても降格時はユーザー通知必須（`> [WARN] worktree 作成失敗 → 順次実行降格`）。
...
実装完了後 `/lint-test` で言語自動判定 + 一括検証（lint / typecheck / test / build）。エラー 0 件 → 完了報告、エラーあり → 自動修正試行。

**自動修正失敗時の挙動**:

| 状況 | 動作 |
|------|------|
| 同アプローチで 2 回連続失敗 | `/clear` 提案して停止、ユーザーに方針再整理要求 |
| `--quick` で haiku 利用不可 | sonnet にフォールバック、軽微修正のみ続行 |
| Serena MCP 失敗 | grep / Read 直接アクセスに降格、warning ログ |

**PushNotification:** 3 分以上経過タスク完了時のみ通知（`"[dev] {task概要} 完了"`）。
