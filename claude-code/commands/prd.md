...
**このフェーズはfastモード・autoモードでもスキップしない。必ずAskUserQuestionで対話する。**

AskUserQuestionで段階的に収集（各Stepごとに1回ずつAskUserQuestionを呼ぶ）:

| Step | AskUserQuestion | 選択肢例 |
|------|----------------|----------|
| 1 | 何を実現したいですか？ | 新規機能 / 既存改善 / 不具合修正 / その他 |
| 2 | どのサービスが関係しますか？ | （コードベースから自動検出した候補を提示） |
| 3 | 外部API・依存はありますか？ | あり（URL入力）/ なし / 不明 |
| 4 | 主な利用者は？ | エンドユーザー / 管理者 / 開発者 / その他 |
| 5 | 主要フローを教えてください | （自由入力を促す）|
...
- **読み取り専用**: 実装はしない。外部APIはWebFetchで取得。Jira/Confluence参照可。反復可能（指摘→修正→再レビュー）。
