...
### Phase 1.8: Writing Context ブロック必須記述（強制機構）

PRD 本文の冒頭に以下のコメントブロックを書き、4問の答えを具体的に埋めてから Phase 2 へ進む。抽象語（「エンジニア」「改善」等）は失格、具体的人物像と数字を書く。

```markdown
<!-- Writing Context（執筆前の4問・書き直しの根拠。最終提出時の削除は任意） -->
<!--
読み手: <PM、実装担当、ステークホルダーの具体名>
読後のアクション: <要件 approve / 追加質問 / スコープ調整 等>
主要な数字・事例: <現状の問題を測った数字、目標 KPI 等>
なぜ必要: <business 背景、ユーザー影響、既存の痛み>
-->
```

### Phase 2: PRD自動生成

概要、ユーザーストーリー、サービス依存関係（Mermaid図）、外部API仕様、状態遷移、受け入れ基準。Writing Context で書いた内容を本文に織り込む。
