...
`~/.claude/references-private/` へ保存先を切替えるか、term を削除 / 匿名化して再実行する。自己除外 file list と詳細判定 logic は `hooks/lib/public-repo-guard.sh` を参照する。incident 経緯: 2026-06-03 commit `8de6a2b` で社内 product 名入り HTML を public push した (social-hit block 未整備時、git history は事後削除で消えない)。block log: `~/.claude/logs/social-hit-block.log`。
