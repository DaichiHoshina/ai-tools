...
- **並列 fan-out 必須 (直列禁止、runtime 実行則)**: 迷ったら並列 + 委譲
  1. 着手前に独立 scope を全列挙して N を確定する。列挙が済むまで 1 発目の Task を発火しない
  2. N≥2 は単一 message に N tool_use を bundle (peak=N)。同一 file の複数修正は patch 並列生成 → 親が順次 apply
  3. 前 agent の結果に依存する逐次のみ可、prompt に `serial_reason: <依存内容 1 行>` を明記する
  4. 各 Task prompt に `scope: i/N` (列挙済み独立 task の i 番目/全 N) を書く。違反検出: `pre-tool-use.sh:_check_developer_agent_bundle_violation`
...
「完了」「動く」「passing」「fixed」等の success 宣言前に、検証 command を fresh に実行して出力を確認する。未実行のまま宣言することを禁止する。canonical: `superpowers:verification-before-completion` skill (enable 済)。**gate 発火 trigger**: (a) commit / push / PR 作成前 (b)「実装した / 修正した / 動くはず / 通るはず」を書く前 (c) subagent の success 報告を採用する前。**gate 動作**: 検証 command を identify → 実行 → 出力全読 → 主張と出力を照合 → 一致したら evidence 付きで宣言する。skip した場合は「未検証」と明示する。ai-tools 側の DoD check hook と jp-quality-block hook の「完了」検出と併走する。
