...
文体 default は plain JP 常体 (canonical: `rules/plain-jp.md`)、質問抑制 default (canonical: `rules/minimize-questions.md`)、思考原則 (事実検証 / 証拠一致 / turn 完結 / 結論先行) の canonical は `rules/thinking-principles.md`。いずれも auto-load 済のため詳細は本 file に書かない。**turn 締め self-check (必須)**: 送信直前に最後の 1 文が「完了 / 〜済 / 次に / 加えて」で終わっていないか、100 字超になっていないかを毎回見直す (7 日 warn 424 / 97 件、rule 記述だけでは効かない実績)。
...
**repo 配下 `.claude/` 配下 doc (rules / skills / workflows / deep 等) の扱い**: 基本 Read しない。repo ごとに「新規実装規範 digest」memory を持ち、そちらを参照して判断する (例: snkrdunk.com → [[snkrdunk-claude-dir-digest]] + 既存修正時の実態は [[snkrdunk-guideline-gap]])。実 code / DB schema / bounded_context 構造は必要時に pinpoint Read (Serena `find_symbol` / repo の `make db-schema` 等経由)。**判断フロー**: (1) 既存 file 修正 → gap file の実態列 + 周辺 code (2) 新規 file 作成 → digest file の規範 (3) 迷ったら参照実装 file を pinpoint Read。一般論 (文体 / 思考原則 / commit / PR / delegation / model 選定) は常に ai-tools 側が正。repo 側 digest がまだない project は、初回に snkr と同じ pattern で digest を作る (agent fan-out で 148 file 規模を 1 session で集約可)。
...
**msg 数 150 超 → `/compact` を自主提案** (200 超は force split 発火で手遅れ)。**>40% と OR 条件**、どちらか先着で発火。
