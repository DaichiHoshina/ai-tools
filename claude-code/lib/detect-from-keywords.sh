...
detect_from_keywords() {
  ...
  if echo "$prompt_lower" | grep -qE '書いて|まとめて|ドラフト|draft|design.?doc|デザインドック|prd|要件定義|要件定|notion|記事|レポート|議事録|報告書|執筆|文章|pr.?description|pr本文|issue本文|rca|障害報告|振り返り|retrospective|プレスリリース|お知らせ|案内文|提案書|adr|技術選定|意思決定記録|ears|受け入れ基準|productspec|techspec|技術仕様書|プロダクト仕様'; then
    _context="${_context}
📝 **執筆検出**: 最優先は「読み手の認知負荷を下げる」「1回読めば理解できる」「で、つまり何？と思わせない」「賢そうでなく伝わる」。

**書く前の4問**: 読み手は具体的に誰／読後の行動／裏付ける数字or事例／なぜ必要。

**守る指針**:
- 用語を並べるだけで説明しない（「可観測性」「疎結合」を並べても内容ゼロ、関係を1文で繋ぐ）
- 暗黙知を前提にしない（「いつもの」「ご存知の通り」を使ったら、その内容を1文で書く）
- 抽象説明の直後に具体例を1つ（数値・事例・コード断片）。抽象だけで段落を終わらせない
- 次の疑問を先回り（1段落書いたら、読み手が次に聞きたいことを次段落に置く）
- 見出しはラベルでなく主張（×「アーキテクチャ」○「読み書きを分離して負荷を分散する」）
- 段落に役割を持たせる（背景/理由/具体例/結論/注意点）。役割が曖昧な段落は削除 or 分割
- AI定型語禁止: 効果的に/シームレスに/革新的な/〜を実現します/〜を可能にします/ご紹介します
- 評価語に根拠1文: 必須/推奨/重要 → 直後に「なぜ・事例・数字」
- 抽象→具体: 大幅改善→「p99 1.2s→320ms (-73%)」
- 短文(issue/PR/Slack)はPREP 400字、長文はTL;DR冒頭1-3文＋常体統一
- 指示語禁止: これ/上記/前述 → 具体名

**避ける段落型**: 抽象ポエム / 用語の羅列 / 過剰丁寧 / 余分形容詞 / 同義反復 / 中身のないまとめ。

**段落 Before/After（抽象→具体への機械的変換）**:
❌ 可観測性向上を目的として疎結合なイベント駆動アーキテクチャを導入し、スケーラビリティを担保する。
⭕ 注文処理が重くなったときに、一部の処理だけ遅延できるようにしたい。そのため、メール送信や通知処理を別の非同期処理に分離する。本処理（在庫引当・決済）の応答時間を p95 500ms 以内に保つことが目的。

**出力直前の自問3つ**: ①読み手が「で、つまり何？」と思わないか ②初見の人が途中で止まらないか ③各段落の役割が明確か。

困ったときのみ claude-code/references/writing-principles.md で詳細参照。常時 Read は不要、注入された指針で完結する。"
  fi
