#!/usr/bin/env bats
# Smoke test: install-warn-log-weekly-cron.sh
#   plist 生成 / worktree guard / --repo override の動作確認。

setup() {
  PROJECT_ROOT="$(cd "$(dirname "$BATS_TEST_FILENAME")/../../.." && pwd)"
  export PROJECT_ROOT
  export SCRIPT_FILE="${PROJECT_ROOT}/scripts/install-warn-log-weekly-cron.sh"
  TEST_HOME="$(mktemp -d)"
  export HOME="$TEST_HOME"
  # bats が worktree 配下から走る場合の guard 解除 (実 worktree test は別 case で挙動確認済)
  export WARN_LOG_CRON_ALLOW_WT=1
}
...
@test "--dry-run は plist 内容を表示するだけ" {
  ...
  echo "$output" | grep -q "com.daichi.warn-log-weekly"
  # plist は配置されないこと
  [ ! -f "$TEST_HOME/Library/LaunchAgents/com.daichi.warn-log-weekly.plist" ]
}

@test "plist コマンドは warn-log-weekly.sh を起動すること" {
  run bash "$SCRIPT_FILE" --dry-run --repo "$PROJECT_ROOT"
  [ "$status" -eq 0 ]
  echo "$output" | grep -qE "\./scripts/warn-log-weekly\.sh"
}

@test "worktree path を REPO_ROOT に渡すと error で stop する" {
  WT_PATH="$TEST_HOME/ai-tools-wt-test/claude-code"
  mkdir -p "$WT_PATH/scripts"
  cp "$PROJECT_ROOT/scripts/warn-log-weekly.sh" "$WT_PATH/scripts/"
  chmod +x "$WT_PATH/scripts/warn-log-weekly.sh"

  unset WARN_LOG_CRON_ALLOW_WT
  run bash "$SCRIPT_FILE" --dry-run --repo "$WT_PATH"
  [ "$status" -eq 2 ]
  echo "$output" | grep -qE "worktree"
}

@test "warn-log-weekly.sh 不在 path で error" {
