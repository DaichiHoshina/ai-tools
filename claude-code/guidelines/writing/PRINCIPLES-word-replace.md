...
**英日混在病の判定**: 「lock」「commit」「deploy」「TX」「group」「writer」等の汎用語は既に日本語訳が定着している。上記の汎用語を英語で残す正当な理由 (読者が SQL log 中の literal を期待している等) がない限り日本語化。表記揺れ (同一文書内で「lock」と「ロック」混在) は最悪、片方に統一。
...
NG list canonical は [NG-DICTIONARY.md](NG-DICTIONARY.md) `AI定型語` key。本表は block 時の書き換え方を示す。多くは**削除して事実だけ残す**のが正解。

| 定型語 | 代替 |
|--------|------|
| 効果的に / 効率的に | 削除し具体指標を書く (「処理時間を 200ms 短縮」) |
| 〜を実現します / 〜を可能にします | 〜する / 〜できる |
| 〜を提供します | 〜する / 〜がある |
| 〜することができます | 〜できる |
| 強力な / 素晴らしい | 削除し事実を書く (「同時 1 万接続を処理」) |
| より良い | 具体差分 (「失敗率を 3% → 0.1% に」) |
| 包括的な | 全項目を網羅した / 一通りそろった |
| 重要なポイント / 注目すべき点 | 削除し要点を直接書く |
| 本機能は / 本記事では / 本稿では | 削除し主語を具体名に (「認証 API は」) |
| まず〜しましょう | 〜する (手順は番号付きリストに) |

**原則**: AI 定型語は情報量ゼロの飾り。消して「何が・どう変わるか」の事実だけ残せば読みやすくなる。
