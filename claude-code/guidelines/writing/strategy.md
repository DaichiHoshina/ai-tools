...
## AI対応ドキュメントルール (RAG検索精度)

embedding生成・AI検索の品質に直結する構造化ルール。
...
| 文量 | 800-1500字 / ページ目安。2000字超なら分割 |
| 見出し階層 | H1-H3まで。H4が必要なら内容が複雑すぎるため分割 |

### 文体・表現

(詳細は [PRINCIPLES.md](PRINCIPLES.md) + [long-form-doc.md NG辞書](long-form-doc.md#ng-辞書-長文向け詳細版) 参照)
...
| ルール | 詳細 |
|--------|------|
| 画像 | テキストで完結するよう言語化 (AIは画像を読めない場合あり) |
| テーブル | セル結合禁止、空欄は「なし」と記入 |
| 色による識別 | 廃止。見出しで区分けする |
| 臨時運用 | タイトル先頭に【臨時運用】+ 有効期限or解除条件を冒頭記載 |
...
Notion詳細仕様は `guidelines/common/notion-writing.md` 参照。
...
`markdownlint` 等のLintツールで強制し、CIで実行。書き手の揺れを機械的に吸収。
