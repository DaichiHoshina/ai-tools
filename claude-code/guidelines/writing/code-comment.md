...
コード内コメント (`//` `/*` `--` `<!-- -->` `#`) の書き方原則。
対象読者: コードレビュアー / 将来の自分 / AI エージェント。
対象外: PR 本文 / commit message (それぞれ専用ガイドライン参照)。

## 基本原則: WHY または重要 memo のみ残す

コメントは「なぜそう書いたか」と「知らないと詰まる情報」に限定する。
...
### (8) commented-out code: 削除予定コードを残さない
...
`// AI-generated` / `// TODO: AI suggested` / `// Generated by Copilot` 等の生成元マーカーは残さない。レビュー時に判断根拠が「AI が書いた」に矮小化され、人間レビュアーが思考停止する原因になる。詳細: `rules/ai-output.md`。
...
抽出したコメントを以下 12 カテゴリで分類し、削除 / 書き直し対象を特定する。
...
- `rules/ai-output.md` — AI 生成マーカー禁止 (`// AI-generated` 等)
