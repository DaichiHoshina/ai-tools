...
書くなら **Why not (なぜ別の選択肢を採らなかったか)** を書く。例外は (a) 調べても辿り着きにくい外部 / 内部運用 memo (`MEMO:` prefix 必須) (b) 公開 API の 1 行 godoc の 2 つだけ。
...
**書き直しで文字数が増えるなら、先に (a) に戻って削除できないかを再判定する** (「丁寧に書き直す」より「消す」が正解のことが多い)。

- **default = 書かない**。迷ったら書かない (コードと識別子で伝わるなら comment は不要)
- **上限 2 行** (Why not 1 文 + 根拠 1 文)。1 行で足りるなら 1 行にする
...
### (3) godoc: 公開シンボルの 1 行要約のみ

公開 API には godoc / JSDoc / docstring 形式で 1 行要約を付ける。private シンボルには原則付けない。実装詳細や特定行の説明を書かない。swagger / OpenAPI annotation もこの分類に含む。
...
**guard: 以下の品質 rule は「書くと決めた 1-2 行」にだけ適用する。** 本節の OK 例は NG 例より長いが、これは「書くと決めた文の直し方」であって「長く書くほど良い」ではない。書き直すと長くなる comment は、その前にまず削除 (what 言い換えなら書き直しではなく削除が正解) を検討する。
...
| godoc (残す) | 公開シンボルの 1 行要約 | そのまま。private / 実装詳細は削除 |
| what (削除) | コードの言い換え | 削除 |
