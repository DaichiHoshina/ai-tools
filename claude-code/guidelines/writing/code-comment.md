...
- 3 行以上を許容するのは incident 由来 workaround / 外部仕様の重要 memo / 説明対象が複雑で 2 行に収まらないもの / **公開 API の godoc** (行数制限を適用しない、後述 §残す 3 分類 (3) 参照)
...
**godoc は「上限 2 行」を適用しない**。godoc は `go doc` / IDE hover / pkg.go.dev で参照される公開仕様 doc であり、code の実装 comment とは目的が別だ。使用例・引数説明・返り値・edge case・関連 API への言及など、利用者が API を正しく使うために要る情報は行数を気にせず書く。ただし冗長さは通常 comment と同じく避ける (書けば書くほど良いわけではない、書き手として不要な行は削る)。
