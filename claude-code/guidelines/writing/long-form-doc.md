...
`/docs` / `/design-doc` / `/prd` / `/retrospective` / `/diagnose` 長文出力時に適用。他人が読むmd / Notion / GitHub保存ドキュメントの執筆原則。

共通文章原則は [PRINCIPLES.md](PRINCIPLES.md) 参照、本ファイルは **長文向けの追加ルール** (Writing Context / 3層リライト / トリアージ / NG辞書 / ADR・PRD・EARSテンプレ) を扱う。
...
PRINCIPLES.mdのAI臭3変換と重複あり。長文では下記3カテゴリでLayer 3検出する。
...
このカテゴリは**削除でなく「根拠併記」が基本**。直後に数字or事例or「なぜ」の1文。

## Before/Afterサンプル (4観点)

| 観点 | Before (AI調) | After (伝わる) |
|------|--------------|----------------|
| 結論先行・根拠 | 本機能は効率的にドキュメント作成を可能にします | Notionへmdを直接投稿できる。手動コピペを無くすのが目的 |
| 難語定義 | CQRSで整合性を担保 | CQRS (読み書きを別モデル分離) で整合性。readはreplicaから返せてscale可 |
| 抽象 → 数字 | エラー処理を強化 | 5xx日次120件 → 8件、`context.WithTimeout` 全handler追加 |
| プロサ繋ぎ | 箇条書き10項目だけ | 「重要度順、最上位X理由〜」の1文 → 3-5項目 → 「残りは参考」の締め |

### プロサ繋ぎ例 (箇条書き前後に地の文)

❌ NG: 「DB分離 / Schema分離 / Row分離」だけ並べる。
...
**注意**: ADRは「過去にこう決めた」記録なので、決定が覆る場合は元のADRを編集せず、新ADRを作って `Status: superseded by ADR-XXX` で上書き。
...
**MoSCoW運用**: 4段で優先順位を明示。「全部Must」は優先順位放棄なのでNG。Won'tを書くと「やらないこと」が合意され、後でスコープ拡大を防げる。
