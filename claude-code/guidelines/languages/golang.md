...
既存の型をコピペして新機能を作るとき、「フィールドが同じだから型も同じでいい」は誤り。ドメイン上の意味が異なるなら、構造が同一でも型は分ける。

| 判断軸 | 同じ型でOK | 型を分けるべき |
|--------|-----------|--------------|
| ライフサイクル | 常に一緒に変更・削除される | 片方だけ変更・削除しうる |
| 命名 | 共通概念を表す名前が自然につく | 無理に共通名にすると意味がぼやける |
| 進化方向 | 将来もフィールドが揃い続ける保証がある | 片方だけフィールドが増減しうる |

**判定基準**: 「この型を変更/削除したとき、無関係な機能に影響するか？」→ Yesなら分ける。

**共通化する場合**: 汎用的な名前を明示的につける（例: `PaginationParams`, `DateRangeFilter`）。特定機能の名前がついた型を他機能で流用しない。
...
| 既存型を別用途に流用 | 用途ごとに型を定義 | 意味的型分離 |
| Usecaseでフィールド直接書き換え | modelに`SetXxx()`メソッドを実装 | 変更ロジックの集約 |
| パラメータなしにInput構造体を作成 | 引数なしにする | 不要な型を増やさない |
| `SELECT *` | 必要なカラムのみ指定 | パフォーマンス・安全性 |
| サーバー側データ不整合に400返却 | 500を返す（400はクライアント起因のみ） | ステータスコードの意味 |
...
- 例外的に生SQL bulk INSERT を書く場合、`LastInsertId() + i` 採番は単純挿入限定（NG: `INSERT...SELECT` / `ON DUPLICATE KEY UPDATE` / 混合モード / 動的行数 / migration backfill）。詳細: [backend/mysql-performance.md §12](../backend/mysql-performance.md)
...
- up/down 両ファイル必須
- 番号はmainの最新を確認してから振る（マージ直前にも再確認）
- down migrationの `MODIFY COLUMN` では `DEFAULT` 句を明示する
