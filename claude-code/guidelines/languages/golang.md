...
| ❌ 禁止 | ✅ 正しい | 理由 |
|---------|----------|------|
| `GetShippedRequest` に `GetCompletedRequest` を流用 | フィールドが同じでも個別に定義 | 独立変更・削除時の巻き添え防止 |
| `GetShippedView` を `GetCompletedView` で代用 | 用途ごとに専用の型を定義 | 将来の型進化を阻害しない |

**判定基準**: 「この型を変更/削除したとき、無関係な機能に影響するか？」→ Yesなら型を分けるべき。

**例外**: 意図的に共通化する場合は共通型として明示的に命名する（例: `PaginationRequest`）。
