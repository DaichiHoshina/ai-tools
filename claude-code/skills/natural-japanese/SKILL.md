...
description: 仕事の日本語文書（議事録・レポート・社内ガイド・企画書・メール・スライド構成案）や note・ブログ・エッセイの執筆・校正・リライトに使う。AI臭さの除去と診断・採点（「AIっぽい」「不自然」「単調」等の指摘全般）に対応する。読みにくい文章の改善（語順・長文・読点・主語述語の距離）、「結論から書いて」等の指示、文体の学習・プロファイル化にも使う。Markdown 整形自体（引用ブロック・脚注記法等）は対象外。
license: MIT
