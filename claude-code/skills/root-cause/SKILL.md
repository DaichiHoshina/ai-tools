...
| カテゴリ | 説明 | 例 |
|---------|------|---|
| **Architecture** | レイヤー違反、コンポーネント欠如 | 検証層がない、依存関係の逆転 |
| **Logic** | アルゴリズムバグ、条件ミス | off-by-one、境界値未処理 |
| **Data** | スキーマ不一致、マイグレーション問題 | 型不一致、NULL制約漏れ |
| **Integration** | API契約違反、外部依存 | レスポンス形式の変更、タイムアウト |
| **Assumption** | 挙動の誤った仮定 | 順序保証なし、冪等性の欠如 |
| **Environment** | 設定、インフラ | 環境変数未設定、リソース不足 |
