...
> **ai-tools 環境の差分**: macOS で運用 (upstream の Windows PowerShell / Linux Bash gate は skip)。`agents/openai.yaml` は Codex 用 metadata で Claude Code 側は無視、`scripts/` の validator と `evaluations/` の versioned case も skill 発火には使わない (upstream 保守用の参考資料として同梱)。
