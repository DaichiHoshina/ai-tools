...
**ゼロ件時の挙動**: Step 1 で全行 hit せず言語が判定できない場合、モード別に common 必須セットを読込（`full` の mandatory ルールを保持）:

| モード | 読込対象 | 報告 |
|--------|---------|------|
| `summary`（既定） | `common/code-quality-design.md` のみ | `detected: none, fallback: common-summary` |
| `full` | `common/claude-code-tips.md` + `common/code-quality-design.md` + `common/development-process.md`（mandatory 3 本） | `detected: none, fallback: common-full` |

いずれもエラーにせず続行。`full` 指定時に common 3 本を skip しない。
