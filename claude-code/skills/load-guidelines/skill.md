...
|-----|-------------|
| TypeScript | `~/.claude/guidelines/languages/typescript.md` |
| TypeScript + ESLint | + `~/.claude/guidelines/languages/eslint.md` |
| Next.js/React | `~/.claude/guidelines/languages/nextjs-react.md` |
| Tailwind | `~/.claude/guidelines/languages/tailwind.md` |
| shadcn/ui | `~/.claude/guidelines/languages/shadcn.md` |
| Go | `~/.claude/guidelines/languages/golang.md` + `go-test-stability.md` |
| Python | `~/.claude/guidelines/languages/python.md` |
| Rust | `~/.claude/guidelines/languages/rust.md` |

**設計（非同期ジョブ検出時）:**

| 条件 | ガイドライン |
|-----|-------------|
| queue/worker/cron等 | `~/.claude/guidelines/design/async-job-patterns.md` |

**バックエンド（サブトピック検出時のみ個別読込、一括投入禁止）:**

トークン節約のため、Backend FW検出だけで6本一括読込はしない。タスクキーワードに応じて該当1-2本のみ追加読込:

| トリガーキーワード | 追加読込ガイドライン |
|------------------|---------------------|
| slow query, EXPLAIN, index, N+1, pool, PostgreSQL | `~/.claude/guidelines/backend/database-performance.md` |
| MySQL, InnoDB, EXPLAIN FORMAT=JSON, buffer pool, GTID, online DDL | `~/.claude/guidelines/backend/mysql-performance.md` |
...
| 言語 | `typescript`, `golang`, `nextjs-react`, `tailwind`, `shadcn`, `python`, `rust`, `eslint`, `go-test-stability`, `go-performance`, `go-concurrency` → `languages/{id}.md` |
| インフラ | `terraform`, `kubernetes` → `infrastructure/{id}.md` |
| 設計 | `clean-architecture`, `ddd`, `async-job-patterns` → `design/{id}.md` |
| バックエンド | `database-performance`, `mysql-performance`, `caching-strategies`, `distributed-transactions`, `observability-design`, `security-hardening`, `scalability-patterns` → `backend/{id}.md` |
