...
| 段階 | 確認 |
|------|------|
| 1-4 全 step 試行後も接続不可 | Claude Code 再起動（設定再読込） |
| 再起動後も不可 | `claude --debug` で起動ログ確認 |
| ログにも手がかり無し | `~/.claude.json` 構文検証（`jq . ~/.claude.json`）|
| 全部 OK だが接続不可 | MCP サーバー実装側の issue 報告（リポジトリ URL を本ファイル「よく使うMCPサーバー」表から特定） |

## 設定例の前提

| 項目 | 動作 |
|------|------|
| 環境変数値が `your-token` 等プレースホルダのまま | 接続失敗、ユーザーに値設定要求 |
| `command` パスが絶対でなく相対 | Claude Code の cwd 依存になり不安定。絶対パス推奨 |
