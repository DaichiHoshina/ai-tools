...
`--type` absent or `all` → `references/` 配下 6 file 全部読む。単一 focus 時は該当 file + `review-criteria.md` のみ (`db-concurrency` → + `silent-failure.md`、`diff-hygiene` → + `diff-hygiene.md` のみ、`review-criteria.md` は不要)。Step 1 前に load する。
