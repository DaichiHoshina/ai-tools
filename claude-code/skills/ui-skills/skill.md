...
UI実装時に手戻りを減らすための必須チェック。

### 実装前確認

| # | 確認項目 | 方法 |
|---|---------|------|
| 1 | 既存UIとの一貫性 | 同プロジェクト内の類似画面を確認し、レイアウト・配色・コンポーネントを揃える |
| 2 | モーダル/ポップアップのサイズ | 内容量に応じた適切なサイズ（大きすぎない） |
| 3 | 表示データの具体性 | IDや番号ではなくタイトル・名前など人間が読める情報を表示 |

### 実装後セルフチェック（コード提出前に必ず実行）
...
| 4 | API所要時間の事前確認 | 重い処理をUI内で同期実行して「時間かかりすぎ」になる |
| 5 | レスポンシブ確認 | ターミナル/ポップアップのサイズが画面に対して不適切 |
