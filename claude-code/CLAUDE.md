...
- **Task Diary**: 以下のいずれかに該当する完了時のみ `/memory-save` を提案: (a) 3ファイル以上の変更 (b) 非自明な設計判断を伴うリファクタ (c) インシデント対応。軽微な修正は `~/.claude/logs/task-diary.log` への自動蓄積で十分
...
**削除されたトリガー**（明示コマンド推奨）: `main push`, `修正してpush`, `秘匿情報確認してpush`, `レビューして直してpush`, `{ブランチ名}にpushして`, `ブランチ切って修正`, `grooveで`, `元に戻して`, `codexで{タスク}`。誤判定リスクとトークン消費を避けるため削除。
