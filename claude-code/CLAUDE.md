...
- **クオリティ最優先 mode**: 破壊的変更 / migration / security 修正は developer-agent (Generator) → reviewer-agent / verify-app (Verifier) の 1 round loop (`status: accept/reject` + feedback → reject 後再生成)
- **並列 fan-out 必須 (直列禁止)**: N≥2 は単一 assistant message に N tool_use を bundle。禁じ手: agent 単発 / 逐次 chain / 複数 message 分散 (いずれも peak=1 落ち)。発火直前 self-check 1 行宣言必須。違反検出: `pre-tool-use.sh:_check_developer_agent_bundle_violation`
- **例外**: 前 agent の結果に真に依存する場合のみ prompt に `serial_reason: <依存内容 1 行>` 明記で逐次可
- **1 dev = 1 file 原則** (`bundle_justification` なき複数 file fan-out 禁止)、1 Task scope 上限 = file 3-5 / 観点 1-2。Model 切替: `/model sonnet` (session 単位)
- **Subagent silent-fail guard**: subagent は `AskUserQuestion` 不可 + permission prompt auto-deny で silent fail する。approval-gated edit / 判断 fork は parent escalate (`status: blocked` + `issues_blocking[]`)。canonical: `agents/developer-agent.md`

詳細 (判定 flow / delegate threshold / parallel fire format / serial_reason 仕様): `references/auto-delegation-detailed.md`
...
commit / PR / issue / MR に人名 / handle / Slack name / 社員 alias を書き込まない (全 repo 共通、Co-Authored-By AI marker は対象外)。**ai-tools は public** のため社内 product / 個人名 / 会社名 / project 固有名詞も `~/ai-tools/` 配下 file と commit message に書込禁止 (hook hard block)。canonical: `rules/public-repo-private-data-block.md` + `guidelines/writing/NG-DICTIONARY.md` (既存 key rename 禁止)。
