...
- **Haiku 4.5**: developer-agent, explore-agent, verify-app（高速処理）

## 探索・調査の使い分け（濫用防止）

subagent-events.log 分析で `general-purpose` 平均115s・最大501s、`Explore` 平均99s と判明。agent 濫用が最大コスト源なので以下ルールを遵守。

| 調査規模 | 使用するツール | 理由 |
|---------|--------------|------|
| 1-2ファイル・特定シンボル | **Bash grep/find または mcp__serena__find_symbol** | agent 起動コスト（数十秒〜）不要 |
| 3-4クエリの広域探索 | **`/explore`（explore-agent×4 並列、Haiku）** | 並列で高速、Haikuで安価 |
| Claude Code CLI/SDK/API の仕様質問 | **claude-code-guide agent** | 専用知識、軽量 |
| それ以外で本当に広域分析が必要 | Explore（built-in） | 最終手段 |

**避けるべきパターン**:
- ❌ `general-purpose` agent（平均115s、最大501s）— 明確な理由がない限り使わない
- ❌ 1ファイル確認のために Explore/explore-agent 起動
- ❌ 3クエリ未満で済む検索に `/explore` 起動
...
上記以外（`main push`、`修正してpush`、`grooveで`、`元に戻して`、`codexで{タスク}` 等）は自然語解釈しない。明示コマンド（例: `/git-push --main`, `/groove`, `/undo`）を使う。誤判定・トークン消費を避けるため。
