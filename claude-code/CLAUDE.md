...
| レベル | 用途 | 例 |
|--------|------|-----|
| error | 処理継続不可、即座の対応必要 | DB接続失敗、外部API障害 |
| warn | 異常だが処理継続可、要監視 | authz.denied、ID指定NotFound（文脈で判断）、rate_limited |
| info | 正常系の重要イベント | リクエスト開始/完了、状態遷移、バッチ処理結果 |

**判断に迷う場合**: 正常系ならinfo、異常だが想定内ならwarn。

### 必須フィールド

| 区分 | フィールド |
|------|-----------|
| 全ログ共通 | msg（固定英語）、event（`order.get`等）、request_id/trace_id、duration_ms、result（success/failed） |
...
|--------|--------|
| 一覧検索0件 | ログ不要（正常系） |
| ID指定NotFound | 文脈でwarn（event: `resource.get.not_found`、suspicion: `possible_id_probe`） |

### warnにすべきセキュリティイベント

`authz.denied`、`resource.get.not_found`（高頻度）、`validation.failed`（高頻度）、`rate_limited`、`auth.login_failed`

### 禁止（ログに入れない）

password、token、Cookie、Authorizationヘッダ、PII生値（要マスク/ハッシュ）、request body丸ごと（field抽出かbody_hash）
