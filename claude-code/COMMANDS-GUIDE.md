...
| **`/review`** | コードレビュー (内部: `comprehensive-review` skill / Team path 時は `reviewer-agent`) | ⭐️⭐️⭐️⭐️ |
...
| コマンド | フェーズ | 用途 |
|---------|---------|------|
| `/brainstorm` | 発散 | 対話的設計精緻化 |
| `/prd` | 要件 | PRD作成（対話式要件整理） |
| `/design-doc` | 設計判断 | DesignDoc 作成（Why/比較/移行/失敗ケース、チーム共有） |
| `/plan` | 計画 | Phase 分解・worktree 判断（Tier 2 にも掲載） |

順序とフェーズ遷移条件: `references/design-phase-flow.md`
...
- [SKILLS-MAP.md](./SKILLS-MAP.md): スキル一覧・使い分け
