...
| ディレクトリ / ファイル | 内容 | 詳細 |
|------------------------|------|------|
| `commands/` | 34 のスラッシュコマンド（`/dev`, `/review`, `/plan` 等） | [docs/commands-quickref.md](../docs/commands-quickref.md) |
| `skills/` | 22 のスキル（comprehensive-review, ui-skills, terraform 等） | 各 `skills/<name>/skill.md` |
| `agents/` | 7 のエージェント（po-agent, manager-agent, developer-agent 等） | [agents/README.md](agents/README.md) |
| `guidelines/` | 67 ガイドライン（言語 / 設計 / インフラ / 運用 / 品質） | カテゴリ別ディレクトリ |
| `hooks/` | 18 のイベント Hook（UserPromptSubmit, PreToolUse, SessionEnd 等） | [hooks/README.md](hooks/README.md) |
...
| `settings/` | MCP server 設定 (`mcp-servers/`) | |
| `config/` | shell utility 設定 (dir-colors 等) | |
| `githooks/` | repo 用 git hooks (pre-commit / pre-push、品質チェック) | |
| `_archive/` | 退避済 (sync 対象外、復活時に commands/ へ戻す) | |
| `CLAUDE.md` | claude-code ディレクトリ固有のグローバル指示 | |
| `VERSION` | Claude Code CLI 本体の追従バージョン（現在 2.1.123）。手動更新（`/claude-update-fix`） | |
