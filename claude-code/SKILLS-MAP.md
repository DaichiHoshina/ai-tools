...
| コマンド | 説明 |
|----------|------|
| `/dev` | 直接実装（--quickでhaiku高速実行） |
| `/flow` | ワークフロー自動化（タスクタイプ自動判定） |
| `/refactor` | リファクタリング（言語ガイドライン自動読み込み） |
| `/debug` | デバッグ支援（エラーログ解析→原因特定→修正提案） |
| `/tdd` | TDD開発モード（RED-GREEN-REFACTOR強制） |
| `/serena` | Serena MCP活用の構造化開発 |
| `/ui` | UI統合（テーマ・実装・レビュー・検証） |

### 設計・計画系

| コマンド | 説明 |
|----------|------|
| `/plan` | 設計・計画（PO Agentで戦略策定） |
| `/prd` | PRD作成（対話式要件整理、10専門家レビュー） |
| `/brainstorm` | 対話的設計精緻化 |
| `/explore` | 並列探索（複数観点から同時調査） |

### レビュー・テスト系

| コマンド | 説明 |
|----------|------|
| `/review` | コードレビュー（comprehensive-reviewで7観点統合） |
| `/review-fix-push` | レビュー→修正→プッシュを1コマンドで |
| `/mr-review` | GitLab MR / GitHub PR レビュー |
| `/test` | テスト作成専用モード |
| `/lint-test` | CI相当チェック一括実行（build/lint/test/typecheck） |

### Git系

| コマンド | 説明 |
|----------|------|
| `/git-push` | Git統合（commit→push→PR/MR作成、モード自動判定） |
| `/commit` | Gitコミットヘルパー |
| `/commit-push-main` | 統合済み→`/git-push --main` |
| `/commit-push-pr` | 統合済み→`/git-push --pr` |
| `/branch-push-mr` | 統合済み→`/git-push --branch` |
...
|----------|------|
| `/docs` | ドキュメント作成 |
| `/retrospective` | 振り返り（セッション分析→改善案提案） |

### ユーティリティ系

| コマンド | 説明 |
|----------|------|
| `/protection-mode` | Protection Mode読み込み |
| `/reload` | CLAUDE.md再読み込み（compaction後復元） |
| `/memory-save` | Serena memoryへの簡易保存 |
| `/serena-refresh` | Serenaデータ・メモリー最新化 |
| `/aliases` | コマンドエイリアス定義 |

> **注**: `/commit-push-main`, `/commit-push-pr`, `/branch-push-mr` は `/git-push` に統合済み。後方互換性のため残存。
