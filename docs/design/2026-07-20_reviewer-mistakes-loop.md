...
- **log**: `~/ai-tools/memory/reviewer-mistakes.md` の git log が実質的な shipping log
- **週次自動集計** (`scripts/verification-metrics.tsv` 経由、`maintenance-cron-run.sh` で発火、集約先は `~/ai-tools/memory/pending-improvements.md`):
  - `reviewer-known-mistakes`: `agents/reviewer-agent.md § Known mistakes` の h3 entry 数
  - `jp-block-overridden`: block 判定が直後 chat で override された rough signal 件数 (`~/.claude/logs/jp-quality-override.log`、`hooks/lib/jp-quality-override-detect.sh` が stop.sh 経由で生成)
- **人手集計 (30 日で 1 回)**: `git log --follow reviewer-mistakes.md` で書き込み回数、`git log agents/reviewer-agent.md` で promote 発火回数、reviewer-agent の実 review 出力で同種 defect 再発回数
- **alert**: なし。B-1 〜 B-5 の判断は 30 日 review turn で user が行う
- **rough signal の限界**: `jp-block-overridden` は「block 対象 term が直近 5 分に literal 含有された」の緩い判定で、意図的 override と偶然含有を区別しない。集計目的専用
