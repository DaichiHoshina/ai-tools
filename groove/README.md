...
```text
groove/
├── workflows/     ワークフロー定義（YAML, version 1）
├── agents/        各ステップで使うAgent定義（Markdown）
├── schema.md      ワークフローYAMLのスキーマ仕様（[詳細](./schema.md)）
...
| 名前 | 流れ | 選択キーワード |
|------|------|---------------|
| spec-driven | 仕様レビュー → Codexレビュー → 実装 → 受入検査 → 修正 → 簡素化 → 検証 | デフォルト |
| tdd | テスト作成 → 実装 → レビュー → 修正 → 簡素化 → 検証 | テスト, TDD, test |
| vsdd | 仕様レビュー → テスト → 実装 → 敵対的レビュー → 修正 → 簡素化 → 検証 | VSDD, 品質重視 |
