...
| 原則 | 意味 |
|------|------|
| Core First, Polish Later | 核心機能を先に動かす。装飾は後 |
| Wire Before You Decorate | データの流れ・接続を先に確立する |
| No Dead Code | 未使用コード・プレースホルダー禁止 |
| The Spec Is Law | 仕様にないものは作らない |
| Built to Grow | 拡張可能な構造にする。ただし投機的抽象化は禁止 |

## 手順

1. 仕様レビューレポートがあれば読む
2. 既存コードを調査し、変更対象を特定する
3. Credoに従い、縦スライス優先で実装する
