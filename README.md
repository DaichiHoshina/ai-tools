...
| 問題 | 解決方法 | 効果 |
|------|---------|------|
| 毎回同じ指示を繰り返す | **27コマンド**で頻出タスクを1行で実行 | ⏱ 時間短縮 90% |
| 技術スタック毎に指示が必要 | **20スキル** + **35ガイドライン**で自動適用 | 🎯 品質向上 |
| 手動でフォーマットや原則を守る | **12Hooks**で8原則を自動化 | 🤖 自動化率 80% |
...
| **PreToolUse Hook** | 3層分類（Safe/Boundary/Forbidden） | 危険操作の自動防止 |
