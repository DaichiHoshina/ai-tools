...
他にも `/test`, `/refactor`, `/docs`, `/flow` など33コマンドを収録。
...
ai-tools/
├── claude-code/
│   ├── commands/        # 33コマンド（/dev, /review, /plan, /prd ...）
│   ├── skills/          # 23スキル（レビュー、開発、インフラ、ユーティリティ）
│   ├── agents/          # 8エージェント（PO, Manager, Developer ...）
│   ├── guidelines/      # 62ガイドライン（言語・設計・インフラ・運用）
│   ├── hooks/           # 18イベントHook
